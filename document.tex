...
the def2-TZVPP basis set with additional polarization functions and finally the 
def2-TZVPPD basis set with both additional polarization and diffuse functions.
...
described by a single-reference method. From there, the $\ccsdtf$ minimization was carried out.}.
...
For $\htoh$, we found \mbox{$\ecc=-0.403\ \kmo$}, so we have an only
weakly bonded system. The CP correction contributes $+0.052\ \kmo$ to
the energy.
...
by $\pw\dt/\tzvpd$ with a deviation of $-0.300\ \kmo$.
...
The opposite problem happens for $\bns$ and $\bns\dt$ optimizations. Here, the attraction is
...
reference energy for the $\htohto$ dimer is at \mbox{$\ecc=-20.804\ \kmo$}, where
the CP correction contributes $+0.177\ \kmo$.
...
\begin{axis}[
    ...
    legend style={legend pos=north east,font=\small,legend columns=3},
    ylabel={interaction energy deviation $\Delta \eint$ / kJ/mol},
...
The inclusion of polarization functions as in def2-TZVPP does
...
more tricky for post-HF methods because some geometries in the optimization process
...
\mbox{$0.097\ \kmo$} and therefore unlikely to deteriorate our results.
The CP corrected interaction energy is \mbox{$\ecc=-6.715\ \kmo$}.
...
since the (reference) water-water interaction energy is of $-20.804\ \kmo$.
