...
\twocolumn[
  ...
  \begin{@twocolumnfalse}
    ...
    \begin{abstract}
      ...
      compared to $\ccsdtf/\vtz$ in a gas phase interaction energy benchmark.
...
%% CHECK: Seit wann?
to a high degree of accuracy. Over the past few decades, it became evident that
the reaction rates governing the formation of molecules can not properly be explained by gas phase
chemistry alone.

A prominent theory to mend this discrepancy is to consider the
contribution of surface reactions on interstellar dust grains. Interstellar dust
was first observed by Trumpler\cite{Trumpler1930}, who attributed extinction
of starlight in the ISM to the presence of dust. Following this first
...
considered proven \cite{Draine2003}.

Apart from dust, a wide variety of molecules is observed in the ISM. In
1976, Merrill \ep{et al.} \cite{Merrill1976} discovered an absorption feature
...
mechanism. Here, molecules X and Y both adsorb on the surface. They diffuse and
when they meet, they react to a molecule Z (or two or more molecules Z, Z',
Z'', \ldots).
...
otherwise it will react further. If the reaction is endothermic or has a
barrier, the energy difference has to be supplied by the surface if it can
not be supplied by the kinetic energies of the molecules
(and neglecting quantum tunneling).
...
\mos. While {\mos} is indeed an accurate and fast functional, in our tests it
had the strange deficiency of convergence failures when unbonded H-radicals
...
and then follows the the direction corresponding to the lowest eigenvalue of the
Hessian matrix to a saddle point (the transition state) and after that reaches
a new minimum by continuing on its route along the gradient. In Figure
...
\cite{DimerMethod}. It uses two images of the system with a slight offset, then
rotates them to find the minimum eigenvalue of the Hessian matrix
\eqref{Theo:Hessian}. It follows these minimum eigenvalue directions by an
