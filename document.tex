...
\twocolumn[
  ...
  \begin{@twocolumnfalse}
    ...
    \begin{abstract}
      ...
      present a $\tzvp/\tzvpd$ hybrid basis set and in combination with a
      ...
      yields similar results to the most accurate $\zpeall$ correction at a
...
serve as an energy sink for reactions that would not usually take place in the
gas phase. The molecules formed by these surface reactions may remain on the
dust grains. If many molecules are accreted on a grain, ices may form that have
...
H+\ho\chemar\hto+\te H$} is dominant \cite{CuppenHerbst2007}.
...
by diffusive processes and when they meet, they have a chance to react to a
molecule Z (or a set of molecules Z, Z', Z'', \ldots).
In the Eley-Rideal (ER) reaction mechanism, only molecule X is adsorbed.
Molecule Y approaches X from the gas phase and the two can react to Z (or more
molecules). In any case, if the reaction is exothermic, the surface has to
absorb the energy difference between reactants and product. If this is
possible, the product Z stays intact, otherwise it will react further or
...
experimental data. While this approach can be justified and does indeed not only
yield good approximations to experimental data but also gives rise to new conclusions,
...
\ep{adsorption energy} of a molecule X on the ice surface S. We call this system $\sX$. All energies appearing are
...
%% CHECK: wird oft ohne Quelle angegeben
...
there are also reactions possible with more than one product, we will, however,
not consider these. Reaction \eqref{Theo:GasReactionScheme} does actually not
take place like this. Instead, a reaction like that can only take place if there
is radiative association, that is a photon of energy $h\nu$ on the left or the
right side of the reaction that maintains conservation of energy. Therefore,
equation \eqref{Theo:GasReactionScheme} is only a formal representation which we
use to maintain analogy to a surface reaction (see below).
...
an \ep{exothermic} reaction. We can also consider the absolute
...
if it is sufficiently clear which energy is meant.
...
the \ep{zero-point (vibrational energy) correction}.
...
be sensible for slowly varying densities. The more universal \ep{generalized
...
terms proposed \cite{BeckeXDM2007,GrimmeDCorrection2010}, of which we will use
...
and then follows the direction corresponding to the lowest eigenvalue of the
...
\textbf{Basis set.} For basis set comparison, we focus on
\ref{Fig:Bench:H2O+H:NoD3}. The basis sets grouped from most attractive to
least attractive are $\svpd$, $\tzvp$, $\qzvp$ and $\tzvpd$. It is obvious that
...
They do therefore all yield good approximations to the optimum geometry. This
...
\begin{table*}[h!]
  ...
  All values are ZPE corrected with the same method. All energies in $\kmo$.}
...
predicts less exothermic reactions than the other functionals.
...
Now we did talk about how we defined the surface geometry and how to decompose
...
\begin{enumerate}
  ...
  rings. Include the hydrogen atoms to arrive at the fully ordered Fletcher
  ...
  dynamics calculation with a classical force field and a finite temperature,
  which should lead to some form of ASW. When the surface satisfies the
  requirements, take a hemispherical cut. You can then add adsorbates or other
  ...
  We used $\tip$ and believe that the force field does not have a very strong
  impact on the result of the calculation due to the large QM region, but for
  smaller QM regions, its effect may be more considerable.
...
\end{enumerate}
Approaching the problem like this was very easy for us, since we used the
chemshell package. It supports a QM/MM interface where the QM and MM theories
can be defined independent of one another. They are then coupled to yield a
hybrid theory, which is given to $\dlfind$ alongside the initial geometry.
...
and the $\tip$ potential, there is an RMSD of around $0.1~\Ang$. The source of
...
with their doublet character, require a sensible choice
...
In general, the value of the corrective term varies from
...
outlook to what other investigations are possible on the surface. 
...
We recommend a central binding site at $(0,2,0)^\intercal$. This is not the
...
hydrogen atoms pointing out of the surface.
...
the optimum geometry yields the activation energy, a key ingedrient to reaction
kinetics.
...
formation scheme \cite{DishoeckHerbstNeufeld2013}.
...
the surface. Values without brackets use the $\zpering$ correction, values in
brackets are uncorrected for ZPE.}
...
I want to thank Professor J. Kaestner for his support, Dr. Thanja Lamberts
